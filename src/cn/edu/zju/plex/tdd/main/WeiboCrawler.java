...
public class WeiboCrawler implements Runnable {
		...
		for(String wuid:targetUsers.keySet()){
			...
			for(int p=1;p<6;p++){
				...
				try {
					...
					try {
						StatusWapper status = tm.getUserTimelineByUid(wuid, paging, 0, 0);
						for(Status s : status.getStatuses()){
							...
							if(Long.valueOf(s.getId())<=Long.valueOf(lastUpdateWeibo)){
								flag = true;
								break;
							}
	...
	public static void main(String[] args) {
		...
		new Thread(new WeiboCrawler(accessToken, targetUsers), "WeiboCrawler").start();
	}
